...
GEMINI_MODEL = genai.GenerativeModel(
    ...
    generation_config={
        ...
        "top_p": 0.95,
        "top_k": 40,
        "max_output_tokens": 3000, # Ensure this is sufficient for your reports
    }
)
        ...
        try:
            ...
            <body>
                <div class="container">
                    ...
                    <div class="footer">
                        <p><strong>Cura Sphere</strong> - Your AI Cura Sphere</p>
            ...
            subject = f'Your Cura Sphere Health Report - {email_context["current_date"]}'
            from_email = 'Cura Sphere Health Assistant <casecompanion07@gmail.com>'
            to_email = [user_email]
...
def handle_general_health(message: str) -> str:
    ...
    prompt = (
        "You are Cura Sphere, a friendly general health assistant. Provide concise, accurate health advice based on the user’s query. "
...
def handle_symptoms_predictor(input_data: str) -> str:
    ...
    prompt = (
        "You are Cura Sphere, an empathetic and knowledgeable medical AI assistant. Analyze the user's input regarding their symptoms. "
        "If age is provided, consider it in your assessment. "
        "Provide a detailed, empathetic, and highly actionable medical assessment rather than a short generic output. "
        "Explain what the symptoms might indicate, possible common causes, and offer clear, practical advice on how to manage them or when to seek immediate medical attention. "
        "Use markdown for bolding and bullet points to make it easy to read. "
        "If the input is unstructured or lacks age, you can gently ask the user to provide their age and more context, but still try your best to analyze the current input. "
        f"User Input: {input_data}"
    )
    return _generate_ai_content(prompt)

def handle_mental_health(input_data: str, request) -> str:
    """Provides mental health support and suggests activities using AI."""
    prompt = (
        "You are Cura Sphere, a mental health assistant. Provide supportive advice based on the user’s input (e.g., feelings like 'sadness' or 'stress'). "
    ...
    if suggested_activity:
        ...
        request.session['last_suggested_activity'] = suggested_activity
...
def handle_disease_predictor(input_data: str, disease_type: str) -> str:
    """Predicts disease risk using loaded ML models and formats response using AI."""
    try:
        inputs = {}
        ...
        if disease_type == "diabetes":
            ...
            prediction_text = "healthy (no diabetes risk)" if prediction[0] == 0 else "at risk of diabetes"
            prompt = (
            ...
            prediction_text = "healthy (no heart disease risk)" if prediction[0] == 0 else "at risk of heart disease"
            prompt = (
                f"You are Cura Sphere, a medical AI assistant. The user has provided their medical parameters: {inputs}. "
                f"Our internal ML model predicts they are '{prediction_text}'. "
                "Please write a comprehensive, empathetic, and detailed response to the user. "
                "Explain what their key parameters mean (e.g. cholesterol, blood pressure, heart rate) in relation to the prediction, and offer actionable lifestyle advice. "
                "Use markdown for bolding and bullet points to make it easy to read. Keep it strictly related to cardiovascular health."
            )
            return _generate_ai_content(prompt)
    ...
    if request.method == 'POST':
        ...
        # Handle to-do commands
            ...
            if message.lower() in greetings:
                bot_response = "Hey there! I'm Cura Sphere, your friendly health coach. Ask me about health or use the mode selector for specific help!"
            elif not message or len(message.split()) <= 2:
                bot_response = "Hey! I'm Cura Sphere, ready to help with your health questions. Try asking something specific or select a mode!"
            else:
                if ai_mode == "general":
                    ...
                    bot_response = handle_mental_health(message, request)
...
def handle_health_report_upload(uploaded_file, user: CustomUser) -> str:
    ...
    try:
        ...
        # Analyze with AI
        prompt = f"""
        You are Cura Sphere, a health report analyzer. Analyze this health report and provide:
